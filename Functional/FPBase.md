...
## 模式匹配
不是因为"高级"或"语法糖"，而是函数式类型系统的消费端，因为它解决了一个核心问题：如何优雅、安全地处理" tagged union"（带标签的联合类型）

例如 Kotlin 的 when 已经比 Java if-else + instanceof 好很多了，但注意那个 else —— 编译器不强制你处理所有分支。但是如果加了一个 Mul 类，eval 不会报错，运行时才会炸
...
// 求值：你要判断类型、强制转换、处理 null
fun eval(e: Expr, env: Map<String, Int>): Int {
    return when (e) {
        is Num -> e.value
        is Add -> eval(e.left, env) + eval(e.right, env)
        is Var -> env[e.name] ?: throw IllegalArgumentException("Undefined: ${e.name}")
...
fn eval(e: &Expr, env: &HashMap<String, i32>) -> i32 {
    match e {
        Expr::Num(n) => *n,
        Expr::Add(l, r) => eval(l, env) + eval(r, env),
        Expr::Var(name) => *env.get(name).expect("Undefined"),
        // 没有处理 Mul？编译错误！
    }
}
```
...
| 函数式特性            | 模式匹配的作用                                                   |
| ---------------- | --------------------------------------------------------- |
| **代数数据类型 (ADT)** | `enum` / `sealed class` 定义数据，模式匹配是唯一安全的消费方式               |
| **不可变性**         | 没有可变状态，数据变换全靠"匹配-解构-重建"                                   |
| **表达式导向**        | 模式匹配是表达式，有返回值，可以嵌套组合                                      |
| **避免 null**      | `Option<T>` / `Maybe a` 用 `Some(x)` / `None` 匹配，编译器强制处理空值 |
| **递归结构处理**       | 树、链表的自然遍历方式                                               |


模式匹配其实是 "case analysis"（情况分析） 的语法化。数学上证明定理要分情况讨论，函数式编程认为代码也应该如此——而且编译器要帮你检查有没有漏情况。
...
1. 代码简洁性  函数式编程通常可以用更少的代码表达相同的逻辑。
2. 可测试性   纯函数易于测试，不需要设置复杂的环境。
3. 并发安全  不可变数据和纯函数天然线程安全，适合并发编程。
4. 可维护性  代码更容易理解和维护，副作用更少。
5. 可组合性  小函数可以组合成复杂的功能。
6. 声明式风格  代码更接近问题描述，而不是实现细节。
