...
在my-big-project这个壳项目的根路径执行 go work init ./app ./common  
这样就可以将 app 和 common 两个独立的 git仓库合并在一起管理和开发，方便跳转  
...
- 当需要从Github上fork一个包并修改了内容及API后，想给自己其他项目依赖时， 就必须要修改这个包的 go.mod 里的 module 改为自己仓库的url 或者将修改内容通过pr入原仓库，否则就无法被使用
  ...
  - 解法： 依赖方直接做replace， 或者 搭建私有proxy

> 例如： A项目想fork一份改点东西，但是 go install 只认 go.mod 导致你自己的仓库没法通过 go install 安装，如果改全部代码的包成本就太大了，这时可以只需要改 go.mod 

```go
// 1. 把模块名字改成你自己的仓库（必须改，否则 go install 认不出来）
module ://github.com

// 2. 重点：加一行 replace，把代码里所有依然写着 original 的旧 import 强行原地纠正
replace ://github.com => ./
```
...
Go 1.14 引入的基于信号的抢占式调度从根本上解决了 Go 1.14 之前纯协作式调度导致的 Goroutine 饿死、GC 延迟以及系统延迟等严重问题。

************************
### 异常处理
> [Go语言的跨协程异常处理](https://taoshu.in/go/goroutine-panic.html)  

Go 的设计崇尚“快失败（Fail-Fast）”。Goroutine 之间在内存上是完全共享同一个堆的，且没有语言级别的“父子级联结构”。如果一个 Goroutine 发生了 Panic 且没有在内部 recover，Go 运行时会认为整个进程的内部状态已经可能遭到不可逆的破坏（例如，某个共享锁可能处于损坏状态），为了防止产生脏数据，干脆直接让整个进程挂掉 [Go Core Team]。

牺牲“局部容错性”，换取“极致的简胜繁”与“高性能”。 Go 故意不引入复杂的父子协程生命周期管理，让协程的创建开销低到极致（只需一个关键字 go）。它把异常防御的责任完全交给了程序员——你必须自己在每个独立的 go func() 内部写好 recover()。
