...
> Git 允许你强制合并两个没有共同祖先的分支。它会把两边的起点直接当成“从空文件开始”，然后将所有文件组合在一起。
- `git merge <对方分支> --allow-unrelated-histories`
...
- `git cherry-pick <提交哈希A> <提交哈希B> <提交哈希C>`
  - Git 会按照你输入的先后顺序，从左到右依次应用这些提交。
- `git cherry-pick <起始提交哈希>^..<结束提交哈希>` 
  - 必须在 <起始提交哈希> 后面加上 ^ 符号。
    - 如果不加 ^：区间是左开右闭 (A, B]，即不包含 A。
    - 如果加上 ^：区间变成左闭右闭 [A, B]，即包含 A 到 B 的所有提交。
...
注意除了用来一个仓库关联多个源（github，gitlab 等）做推送，还可以将完全没关系的两个仓库直接关联起来，也可以做 rebase merge的行为  
例如一个项目A 派生出了 B 和 C 两个业务场景，但是A自己也还在优化，三个仓库都是独立的git root commit，这个时候 B和C可以将A的remote关联进来，然后直接将A的修改合入。  

还可以
- 跨库 Push（强推/创建新分支）：  你可以把 A 库的历史，直接推送到 B 库变成一个全新的、独立的分支。git push remote-B local-branch-from-A:new-branch-in-B
- 制作 Submodule（子模块）或 Subtree（子树）： 将 A 库的整个历史作为 B 库的一个子目录嵌入进去，实现项目嵌套。
- Diff（对比差异）： 直接对比两个毫无关联的项目在当前节点的代码差异。git diff branch-A branch-B
- Log 混合查看： 在一个终端里，同时查看两个独立项目的演进历史拓扑图。git log --oneline --graph --all
