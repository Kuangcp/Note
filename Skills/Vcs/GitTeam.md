...
- 1. [使用Git进行团队协作](#使用git进行团队协作)
    ...
    - 1.3. [自建Git服务](#自建git服务)
    - 1.4. [Tips](#tips)
        - 1.4.1. [master作为线上分支时，误提交功能并推送怎么处理](#master作为线上分支时误提交功能并推送怎么处理)
- 2. [GUI](#gui)
...
- [Forgejo – Beyond coding. We forge.](https://forgejo.org/)`Go开发`  
...
[普通分支模型的回滚](/Skills/Vcs/GitBase.md#分支合并)`分支间只有merge`

`特殊背景： dev分支是merge，上线时是将所有提交sqush成一个提交，直接在master提交`  
好处： master 分支比较干净  
坏处： 业务分支和master没有公共祖先，要先去复杂的dev分支  
