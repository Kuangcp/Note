...
> [IRC协议： OFTC实现](https://www.oftc.net/WebChat/)  
...
### FTP
整体设计比较简单，通常区分两个模式 主动和被动模式

主动模式
1. 客户端连接服务器的 21 端口（控制连接）
2. 客户端发送 PORT 命令，告诉服务器自己的 IP 和端口
3. 服务器从 20 端口主动连接客户端指定的端口（数据连接）
4. 传输数据
...
2. 客户端发送 PASV 命令
3. 服务器响应一个临时端口号
4. 客户端主动连接服务器的临时端口（数据连接）
5. 传输数据

| 特性 | 主动模式 | 被动模式 |
|:---|:---|:---|
| 数据连接发起方 |	服务器主动连接客户端| 	客户端主动连接服务器
| 服务器端口 |	固定使用 20 端口 | 	使用随机临时端口
| 防火墙友好性 |	对客户端防火墙不友好| 	对服务器防火墙不友好
| NAT | 兼容性| 	在 NAT 后的客户端有问题 | 	更好的 NAT 兼容性
| 安全性 |	服务器需要能连接任意客户端端口| 	客户端只连接服务器端口

而且FTP的TCP连接通常设有超时时间，由服务端控制，可以通过发送NOOP指令做心跳保持。 
例如在批量删除一堆文件时，只执行DELE命令，可能被服务端认定为空闲连接被关闭客户端抛出Socket异常
...
如果想要实现安全的 通过公网中转内网的流量， 例如做私有化部署时，可以使用 stcp 隧道方式: 拓扑结构是 frpc 内网 <-> frps 公网 <-> frpc (访问内网的一方)  
如果访问方没法运行frpc搭建起stcp隧道，就在frps端 起一个 ttyd 或 tty2web 暴露出公网地址 注意加上帐号密码
