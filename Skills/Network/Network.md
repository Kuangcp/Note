...
OSI制定的OSI七层参考模型的过于庞大、复杂。与此对照，由技术人员自己开发的TCP/IP协议栈(四层 网络接口 网络层 传输层 应用层)获得了更为广泛的应用。  

谢希仁所著的 计算机网络 书中 为了教学, 融合成了 `五层模型`: 网络接口层 等价于 物理层 + 数据链路层 
...
| 127 | 非全0或全1的任何数 | 可以 | 可以 | 作本地软件回环测试用 |
