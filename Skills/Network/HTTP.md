...
- 2. [HTTP各个实现版本](#http各个实现版本)
    ...
    - 2.3. [HTTP/1.1](#http11)
    - 2.4. [HTTP/2](#http2)
        - 2.4.1. [Cleartext Handshake](#cleartext-handshake)
    - 2.5. [HTTP/3](#http3)
        - 2.5.1. [WebTransport](#webtransport)
- 3. [HTTPS](#https)
...
HTTP： 类似HTTP升级WS，通过 Upgrade header来实现切换协议
HTTPS：依赖TLS握手阶段 ALPN扩展
...
定义：QUIC 里的“多流”是什么
- 1 条 QUIC 连接（UDP 四元组）可并行开出 N 条双向流（Stream）
- 每条流有独立 ID、滑动窗口、收发状态；可同时存在可靠、半可靠、不可靠三种语义
- 流之间无 TCP 式队头阻塞：丢包只影响本流，其他流继续传输

“多流”=一条 QUIC 里并行跑多条独立管道；优势是无队头阻塞、低延迟、高并发，代价是实现复杂；凡是要“既快又稳还实时”的场景——网页、会议、游戏、远程控制——都值得开启多流

### WebTransport
基于 QUIC/HTTP3 的“多通道传输框架”——既能像 WebSocket 一样跑可靠有序流，也能像 UDP 一样发不可靠数据报，天然避免队头阻塞

| 维度          | WebSocket (TCP) | WebTransport (QUIC/UDP)                                                   |
| ----------- | --------------- | ------------------------------------------------------------------------- |
| **握手时延**    | 1× RTT（TCP+TLS） | 0× RTT（复用 HTTP/3 会话）                                                      |
| **队头阻塞**    | 有（TCP 流级别）      | **无**（QUIC 多路复用）                                                          |
| **传输语义**    | 仅**可靠有序**流      | **可靠流**+**不可靠数据报**+**多并发流**                                               |
| **并发能力**    | ~6–10 流         | **理论上百条流**                                                                |
| **弱网表现**    | 丢包重传，延迟飙升       | **前向纠错+更激进重传**，延迟低 40–50%                                                 |
| **API 复杂度** | 事件驱动，简单         | **流式+数据报双模式**，学习成本更高                                                      |
| **浏览器生态**   | 100 % 支持        | **Chrome/Edge≥97 完全支持**；Safari≥16.4 **部分支持**（无服务器推送流）；Firefox≥114 **实验性** |
| **服务器生态**   | 任意语言，成熟         | **.NET 10 / Node.js / Go 已有生产级实现**；其余语言需封装 QUIC                           |


************************

# HTTPS
> [SSL & TLS](/Algorithm/Cryptography.md#ssl-tls)
...
## SSL 握手
旧版本HTTPS都是使用SSL，但是现在基本都是使用TLS 的 1.2 1.3，口语惯例还是称为 SSL握手

> [httpstat](https://github.com/davecheney/httpstat)  可以看到先DNS，然后TCP，然后TLS
