...
- 2. [应用场景](#应用场景)
    ...
    - 2.4. [分布式缓存](#分布式缓存)
        ...
        - 2.4.2. [Apache Geode](#apache-geode)
- 3. [场景](#场景)
    - 3.1. [Hot Key](#hot-key)
    - 3.2. [BigKey](#bigkey)
...
核心定位 = “内存级实时数据库 + 分布式缓存 + 消息总线” 三合一。

| 维度        | 一句话说明                             | 区别于 Redis                 |
| --------- | --------------------------------- | ------------------------- |
| **内存网格**  | 数据默认驻内存，支持 **磁盘持久化/异步写后** 双模式     | Redis 纯内存或简单 RDB/AOF      |
| **分布式事务** | **ACID 跨节点事务**（两阶段提交），强一致         | Redis 仅单节点事务              |
| **数据拓扑**  | **分区 + 复制** 自动均衡，**动态扩缩**不停服      | Redis 需手动分片/哨兵            |
| **事件驱动**  | **连续查询 CQ + 保证消息传递**（至少一次）        | Redis 无内建消息总线             |
| **查询能力**  | **OQL 对象查询 + 函数式计算**（可分布式跑）       | Redis 仅键值/简单聚合            |
| **生态**    | **Spring Data Geode / Boot 一键集成** | Redis 也需 Spring Data，但功能薄 |


| 场景               | 怎么用                                             | 代表客户                   |
| ---------------- | ----------------------------------------------- | ---------------------- |
| **实时风控 / 交易撮合**  | 行情、订单、风控指标全放内存，**毫秒级计算 + 事务落盘**                 | PayPal、Bank of America |
| **高并发会话 / 用户画像** | 社交平台把 **用户资料、关注列表** 放 Geode 分区，**千万级并发读 <1 ms** | Salesforce、Expedia     |
| **实时推荐 / 搜索缓存**  | 热商品、热搜词 **连续查询** 推送到边缘节点，**动态更新**               | eBay、Zillow            |
| **车辆/设备遥测**      | 车联网把 **车况、故障码** 写 Geode，**连续查询触发告警**            | Ford、Cisco             |
| **医疗/保险大数据**     | 理赔规则、病历摘要 **分区存储 + 函数式聚合**，**秒级出单**             | UnitedHealth Group     |
