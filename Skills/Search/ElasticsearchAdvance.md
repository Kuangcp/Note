...
- 大批量写入前
   - 设置副本为0写入后再调整副本避免资源竞争
   - 设置刷盘周期 refresh_interval 例如1h 或者 -1 直接关闭落盘（规避IO风暴，带来更高写入吞吐量，也代表更大内存压力，所以需要权衡利弊）  
- 增量新增和更新时： 按实际业务

设置长达 1h 的 refresh_interval 确实能极大缓解 IO 风暴，但那通常是“离线数仓”或“大批量离线导入”的极端优化，并不适合“交互式搜索”场景。

不同场景的最佳实践建议

| 场景 | 推荐配置 | 理由 |
|---|---|---|
| 标准生产环境 | 1s (默认) | 兼顾实时性和性能。大部分搜索应用的首选。 |
| 高频写入/日志场景 | 30s 或 60s | 显著减少小段（Segment）合并压力，提升写入吞吐。 |
| 大批量数据初始化 | -1 (禁用) | 彻底关闭刷新，此时导入速度最快。导入完后再改回 1s。 |
| 调整索引结构时 | 1s 或 手动调 | 结构调整期间必须设短，否则你根本无法验证 Mapping 是否生效。 |

------------------------------
如何在“避免 IO 风暴”和“可见性”之间权衡？ 如果你担心 IO 风暴，可以采用 “动态调整法”：
> 方案 A：批量更新模式 (针对你现在的任务)

   1. 临时关掉: PUT /index/_settings { "refresh_interval": -1 }
   2. 暴力刷数: 执行你的 _update_by_query。此时没有 refresh 干扰，冲突会变少，IO 也稳定。
   3. 手动刷新: 执行 POST /index/_refresh。
   4. 恢复正常: 改回 1s。

> 方案 B：增加 Index Buffer (更专业的做法)

如果你想保留 1s 的实时性，又想抗住 IO 压力，不应该动 refresh_interval，而是去调 ES 集群的内存配置：

* 调大 indices.memory.index_buffer_size（默认是堆内存的 10%）。
* 这样内存能 hold 住更多数据，减少强制刷盘。
