...
在早期的 Linux 内核中（内核版本 < 5.6），当系统进行密集加密操作（如生成 `gpg` 密钥对、`tomcat` 启动时生成会话 ID、`java` 加密算法调用）时，会调用 `/dev/random` 设备获取高质量随机数。  
- **现象：** 如果系统缺乏高频外部硬件交互（如无机房托管的无头 headless 服务器或虚拟机），内核收集不到足够的硬件噪声，熵池计数（`entropy_avail`）会迅速枯竭并降到非常低的值（甚至个位数）。
- **后果：** 此时 `/dev/random` 会**强行阻塞**应用程序的运行，直到内核重新收集到足够的噪声比特。这会导致 Tomcat 启动卡死几分钟、GPG 报错或生成缓慢。


可使用 [rng-tools](https://wiki.archlinux.org/index.php/Rng-tools) [Github](https://github.com/nhorman/rng-tools) 守护进程来补充熵池

### rng-tools 的实现原理
`rng-tools` 作为一个用户态的守护进程，其核心逻辑是：**“寻找并提取硬件随机数，对其进行统计学质量检测，然后源源不断地下喂给 Linux 内核熵池”**。

具体运作流程如下：
1. **寻找硬件伪随机数源（HWRNG）：** `rngd`（`rng-tools` 的核心进程）在启动时会检测 CPU 和主板是否提供了硬件级别的随机数生成器。
   * **x86 CPU 的 RDRAND 指令：** 现代 Intel 和 AMD CPU 内部都有一个硬件级真随机数发生器（利用热噪声或电子器件亚稳态原理）。`rngd` 可以直接利用特殊的汇编指令（如 `RDRAND` / `RDSEED`）快速批量获取硬件随机数。
   * **其他硬件源：** 如 TPM 芯片、主板上的硬件随机数卡（如 `/dev/hwrng`），甚至是音频输入端输入的低噪。
2. **FIPS 140-2 统计测试（可选，但核心）：**
   从硬件源读取数据后，`rng-tools` 默认会采用 FIPS 140-2 密码学标准中的 4 种统计测试方法（Monobit、Poker、Runs、Long Runs）对获取到的比特流进行实时测试。
   * **目的：** 确保这个硬件生成器没有坏掉、没有输出一串全为 `0` 或 `1` 的规律数据，验证其“真随机性”。
3. **注入内核熵池：**
   通过测试的高质量硬件随机数，`rng-tools` 会调用 Linux 的专用底层接口（通过向 `/dev/random` 写入数据，或使用 `ioctl(RNDADDENTROPY)` 系统调用）直接对内核熵池实施“饱和式注入”，从而让 `entropy_avail` 保持在最大值。

### 现代 Linux 内核（Kernel 5.6+）的重要机制演变

如果你使用的是较新的发行版或滚动更新系统，你会发现：**即便不装 rng-tools，`entropy_avail` 也永远是 `256` 并且 Tomcat 再也不卡顿了。** 

这是因为 Linux 内核在近年做出了巨大的密码学重构：
1. **统一了内部结构：** 在旧内核中，`entropy_avail` 满分是 `4096`。而在新内核中，内核引入了 **BLAKE2s 哈希函数** 和 **Chacha20 流密码** 来重构随机数发生器（CSPRNG）。新算法彻底摆脱了早期基于 SHA-1 计数器的低效方式，现在熵池的上限硬编码为了 **`256`**（代表达到 256 比特的最大安全安全强度）。
2. **不再引发物理阻塞：** 从内核 5.6 开始，`/dev/random` 的行为被重构：它**不再具有无限期的阻塞性**。只要系统在开机阶段（Boot Time）成功完成了一次初始化（通常仅需从 CPU 的 RDRAND 或时钟抖动中收集到 256 比特的初始种子），此后的 `/dev/random` 就会向外部调用源源不断地提供伪随机数，绝不因缺少后续噪声而将程序挂起。
3. **更优的平替：haveged：**
   在较老的没有 `RDRAND` 硬件指令的虚拟化/老旧机器中，通常更推荐用 **`haveged`**。它通过监测 CPU 执行特定代码时由于分支预测、高速缓存（Cache）未命中引起的**CPU时钟抖动（HAVEGE 算法）**在内存中制造随机数。它完全不需要任何硬件支持，性能极高。
