...
在传统架构中，编写文件系统（如 ext4、NTFS）必须写进 Linux 内核，一旦出错整个系统就会崩溃（Kernel Panic）。而 FUSE 允许开发者在普通的用户空间用任何语言（Python、Go、Java、C）编写文件系统逻辑，只要把数据包装成“文件夹和文件”的模样即可。

> FUSE 创意应用场景大盘点
- 把“网络云盘”伪装成“本地硬盘”（最实用） 这是 FUSE 最成功的商业与开源应用。它能将远程的无限网络存储，直接挂载为本地电脑里的一个物理盘符，读写云端文件就像读写本地硬盘一样流畅。
- 无感知的“绝密隐形保险箱”（隐私安全）在用户空间内，FUSE 可以在文件被写入磁盘的前一毫秒对其进行拦截和魔改。
- 跨国/跨网“文件任意门” SSHFS：通过 SSH 协议将千里之外的服务器目录瞬间挂载到本地。它的底层正是基于 FUSE。有了它，你不需要用 ftp 软件，本地的开发工具（如 VS Code、GoLand）可以直接双击打开服务器上的代码进行实时编写和保存。
- 把“吃灰的社交软件”变成“无限免费网盘”很多社交软件或代码托管平台提供了无限的附件上传或存储空间，极客们利用 FUSE 将这些平台的 API 封装成了硬盘。


卸载命令: fusermount3 -u /mnt/fs1
...
安装系统时，如果没有将/tmp指定独立的分区，将会在/分区下建立 tmp 目录，此时会有一个隐患 当系统段时间大量创建tmp文件时**可能导致 / 分区满掉**，从而导致整个系统hang住。  
例如Java中使用EasyExcel大量导出Excel时，会用临时文件落盘避免Java进程的内存OOM的问题, 当并发大量文件导出时容易引起tmp目录占满。  
