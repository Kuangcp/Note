...
- 4. [垃圾收集器](#垃圾收集器)
    ...
    - 4.7. [CMS](#cms)
    - 4.8. [G1](#g1)
        - 4.8.1. [JDK26 JEP 522 到底优化了什么？](#jdk26-jep-522-到底优化了什么)
    - 4.9. [ZGC](#zgc)
...
目前Python仍使用该算法，为了解决循环引用问题就： 标记-清除（做消环 减引用，类似可达性分析了）  并使用 分代回收 提升GC效率
...
| Parallel Old (第二代) | |
| CMS (第三代) | |
| G1 (第四代) | Region复制无碎片，标记可暂停和恢复 吞吐可控，主动选低存活Region可预测停顿 |
| ZGC/ShenandoahGC | 染色指针+读屏障 全部阶段并发，停顿压到亚毫秒级 | 
...
JDK14发布时被移除，因为有了更好的替代GC，G1

`-XX:+UseConcMarkSweepGC`

工作流程：
1. `初始标记` CMS initial mark  **STW** 但是耗时很短 几十ms
...
1. 无法处理浮动垃圾, 可能出现 Concurrent Mode Failure 从而引起新一次FullGC SerialOld，大堆时STW很久
    - 并发清理阶段用户线程还在运行，这段时间就可能产生新的垃圾，新的垃圾在此次GC无法清除，只能等到下次清理
1. 由于使用的是标记清除算法（为了并发复杂度低而不做整理）, 容易导致大量空间碎片, 这样的后果是分配大内存对象会很麻烦, 往往出现老年代总空间还有大量剩余, 但是没有足够大的连续空间
