...
- 5. [实践](#实践)
    - 5.1. [任务建模](#任务建模)
    - 5.2. [ConcurrentHashMap](#concurrenthashmap)
    - 5.3. [ConcurrentSkipListMap](#concurrentskiplistmap)
    - 5.4. [CopyOnWriteArrayList](#copyonwritearraylist)
    - 5.5. [Queue](#queue)
...
该关键字对应于字节码中的指令是 monitorenter 和 monitorexit 

- 只能锁定对象，不能锁定原始类型，因为需要对象头来存储锁状态。
- 锁的范围要尽可能的小，尽量降低竞争
- 被锁定的对象数组，其中的单个对象不会被锁定
...
锁：wait() notify() 只能和 synchronized 一起用，Condition实现同样功能会更灵活，因此推荐使用
...
# 现代并发JUC包
> 简称为J.U.C (java.util.concurrent)

> [AQS](/Java/AdvancedLearning/Concurrency/AQS.md) 

建议通过使用`线程池`,`Task(Runnable/Callable)`,`读写锁`,`原子类`和`线程安全容器`来代替传统的同步锁,wait和notify, 提升并发访问的性能, 降低多线程编程的难度, Netty就是这么做的

> 线程安全容器底层使用了CAS,volatile,和ReadWriteLock实现

- ReentrantLock 和 sync 加解锁机制的区别?  
    - 一个作用于线程一个作用于临界变量
- 不要依赖线程优先级,语言和操作系统级别都是不准确的
...
> [Java 常见并发容器总结 | JavaGuide](https://javaguide.cn/java/concurrent/java-concurrent-collections.html)  
