...
- 4. [Lambda](#lambda)
    ...
    - 4.4. [类型检查、类型推断以及限制](#类型检查、类型推断以及限制)
        - 4.4.1. [类型检查](#类型检查)
        - 4.4.2. [一个Lambda兼容不同的函数式接口](#一个lambda兼容不同的函数式接口)
        - 4.4.3. [类型推断](#类型推断)
        - 4.4.4. [使用局部变量](#使用局部变量)
    - 4.5. [复合 Lambda 表达式](#复合-lambda-表达式)
        - 4.5.1. [Comparator 比较器复合](#comparator-比较器复合)
        - 4.5.2. [Predicate 谓词复合](#predicate-谓词复合)
        - 4.5.3. [函数复合](#函数复合)
...
1. 单一属性比较 `Comparator<Apple> c = Comparator.comparing(Apple::getWeight);` (实用的**Comparator.comparing**方法)
    - 默认顺序(小到大) 逆序追加 reversed()
    - 例如 按重量逆序排序, 重量一致时则再按国家排序 `inventory.sort(comparing(Apple::getWeight).reversed().thenComparing(Apple::getCountry));`
...
> [java8 stream运行原理之并行流原理详解-CSDN博客](https://blog.csdn.net/weixin_38308374/article/details/112735120)  
