...
> [java中的堆、栈、常量池以及String类型的两种声明 - LeonNew - 博客园](https://www.cnblogs.com/LeonNew/p/5314731.html)  
...
常量池中主要存放两大类常量：
1. **字面量 (Literals)**：如文本字符串（`"hello"`）、声明为 `final` 的常量值、基本数据类型的值。
2. **符号引用 (Symbolic References)**：包括类和接口的全限定名、字段的名称和描述符、方法的名称和描述符。

Java 架构中实际上有三个处于不同阶段、不同位置的“常量池”：

| 常量池类型 | 存在位置 | 存在生命周期 | 核心作用 |
| :--- | :--- | :--- | :--- |
| **Class 文件常量池**<br>(静态常量池) | 磁盘上的 `.class` 文件中 | 编译期生成，存放在磁盘 | 存储该类编译后的字面量与符号引用。 |
| **运行时常量池**<br>(Runtime Constant Pool) | 内存中的**元空间** (Metaspace) | 运行期，类加载后存在 | 类加载时，JVM 将上述静态常量池读入内存，并将**符号引用**解析替换为**直接引用**（真实的内存地址）。 |
| **字符串常量池**<br>(String Constant Pool) | 内存中的**堆** (Heap) | 运行期，全局共享 | 专门为了全局复用 `String` 字面量，避免重复创建对象造成的内存浪费。 |


在阅读老博客或旧书时，关于常量池的位置有很多自相矛盾的描述，请以现代 JVM 行为为准：
* **Java 6 及以前**：运行时常量池和字符串常量池都在**永久代 (PermGen)** 中。如果大量使用 `intern()` 很容易导致 `java.lang.OutOfMemoryError: PermGen space`。
* **Java 7**：将**字符串常量池**和**静态变量**从永久代移出，放到了 **Java 堆 (Heap)** 中。
* **Java 8 及以后**：彻底废除永久代，引入**元空间 (Metaspace)**（直接使用物理内存）。**运行时常量池**移入元空间；而**字符串常量池**依然保留在 **Java 堆 (Heap)** 中。
