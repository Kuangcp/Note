...
- 应用程序中预期使用的内存量是否仅包含 Java 堆大小？这个总和是否考虑了 JVM 或应用程序消耗的实际RAM？ 
    - 使用的 RAM 量如何计算？ 该统计信息是否会考虑操作系统报告的 JVM 进程驻留内存大小？Java堆上当前数据的数量是否也包括在内？`RAM占用量严格来说包含 堆，堆外，元空间，二进制库，注意堆外有大文章`
...
> [JVM工具： JMC](/Java/AdvancedLearning/JvmTool.md#jmc)  

- 堆内 用JVM工具进行分析，结合GC日志
- 堆外
    - [堆外内存](/Java/AdvancedLearning/JVM.md#directmemory-堆外内存)  
    - 如果是K8S对pod内存设限导致的kill，还需考虑 cache/buff 占用高的情况
...
- [APM类监控系统](/Skills/Ecology/APM.md)。例如：CAT等
...
### 线程
1. [Jstack 查看栈](/Java/AdvancedLearning/JvmTool.md#jstack)  
1. [Fork Join 排查手册](/Java/AdvancedLearning/Concurrency/ForkAndJoin.md#tuning)
