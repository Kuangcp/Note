...
2. 定位到对应的桶: `tab[(n - 1) & hash]`
3. 遍历链表或红黑树查找目标节点
4. 找到节点后执行删除操作
5. 删除后判断是否需要进行反树化（链表长度 < 6 时）
...
- `matchValue` 参数: 为 true 时只有 value 也匹配才删除（用于 `remove(key, value)` 方法）
- 删除红黑树节点后会调用 `afterNodeRemoval` 进行平衡调整
- 删除后如果桶内节点数减少到阈值以下，不会立即反树化，等待后续操作触发
