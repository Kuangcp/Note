...
- 1. [索引](#索引)
    - 1.1. [Explain](#explain)
    - 1.2. [SHOW PROFILE](#show-profile)
    - 1.3. [为何选择 B+ 树结构](#为何选择-b+-树结构)
    - 1.4. [基本DDL](#基本ddl)
- 2. [索引的类型](#索引的类型)
    - 2.1. [唯一索引](#唯一索引)
    - 2.2. [主键索引](#主键索引)
    - 2.3. [主键索引/聚簇索引](#主键索引聚簇索引)
    - 2.4. [辅助索引/非聚簇索引](#辅助索引非聚簇索引)
    - 2.5. [Hash 索引](#hash-索引)
        - 2.5.1. [AHI 自适应哈希索引](#ahi-自适应哈希索引)
    - 2.6. [全文索引](#全文索引)
    - 2.7. [前缀索引](#前缀索引)
- 3. [索引使用场景](#索引使用场景)
    - 3.1. [索引优化](#索引优化)
...
> 注意Explain返回的是**可能**的查询器和优化器的执行策略。  
...
> 覆盖索引
- 覆盖索引（covering index，或称索引覆盖），即`仅从辅助索引中就可以得到查询的信息`，而不需要查询聚簇索引甚至回表。
...
聚簇索引：指该索引决定了表中数据行**物理存储**排序方式的索引，因此`一个表只能有一个聚簇索引`

而聚簇索引（clustered index）就是按照每张表的主键构造一棵B+树，同时叶子节点中存放的即为`整张表的行记录数据`，也将聚簇索引的叶子节点称为数据页。  
聚簇索引的这个特性决定了索引组织表中数据也是索引的一部分。同B+树数据结构一样，每个数据页都通过一个双向链表来进行链接。
...
| ------------- | -------------------- | ---------------------------- |
| **有主键**       | 用户定义的 PK             | 叶子即数据页                       |
| **唯一索引**      | 第一个非空唯一索引            | 同上，叶子即数据页                    |
| **无主键、无唯一索引** | **隐含 RowID**（6 B 自增 上限值 2^48） | 叶子是 RowID→行数据；**用户索引全是二级索引** |

## 辅助索引/非聚簇索引
辅助索引（Secondary Index，也称非聚簇索引）

叶子节点并不包含行记录的全部数据，只包含索引列 和 主键值（回查主键索引），辅助索引的存在并不影响数据聚簇索引及数据存储，因此每张表上可以有多个辅助索引。  
...
非聚簇索引的使用场合为 查询所获数据量较少时 或者 某字段中的数据的唯一性比较高时， 非聚簇索引必须是稠密索引`SQL查询的字段大部分有建索引`
...
```
-- 创建前缀索引（只索引前 191 个字符）
ALTER TABLE 表名 ADD INDEX idx_字段名 (字段名(191));
```

PG 的索引限制是 单个索引项约 8KB（页大小限制），但远超 MySQL 的 3072 字节，且支持 TOAST 机制 自动处理大字段。

缺点：
- ORDER BY 、GROUP BY 不准确，因为只保证了索引的字符长度的有序性
- 如果是唯一索引，唯一性约束失效
- 覆盖索引失效，需要回表
- LIKE 查询 后缀/中间匹配完全失效，不走索引
- 如果前缀数据区分度低，会降低索引选择，引擎走全表
...
3. order by ，group by ，distinct 的字段需要添加到索引。

总结： 查询，排序 能利用索引 避免全表扫描和FileSort
