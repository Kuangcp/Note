...
| 类型           | 最大字节数           | 约等于     | 额外开销 |
| ------------ | --------------- | ------- | ---- |
| `TINYTEXT`   | 255 B           | 0.25 KB | 1 B  |
| `TEXT`       | 65 535 B        | 64 KB   | 2 B  |
| `MEDIUMTEXT` | 16 777 215 B    | 16 MB   | 3 B  |
| `LONGTEXT`   | 4 294 967 295 B | 4 GB    | 4 B  |

> 注：UTF-8 下每个字符最多占 4 B，因此字符数约为表中字节数的 1/4 左右

TEXT 默认 256 byte 指针留在主记录，>768 byte 的剩余部分放到外部溢出页；
LONGTEXT 同理，只是“允许指针指向的链表”更长；
副作用：

    如果列内容 < 40 KB 且查询只读其它字段，主记录 256 B 指针不变，性能几乎无差异；
    一旦 频繁 SELECT 该列 且内容几十 KB↑，每次都要多读 1-N 个溢出页，逻辑读翻倍

内容长度”本身不会拖慢索引，但只要频繁访问、复制、备份，LONGTEXT 的溢出页、binlog 镜像、网络包拆片会把 CPU、内存、带宽、从库延迟 全部放大；
< 64 KB 且很少读整列 → 直接改； > 几百 KB 且高频读/改 → 拆表 + 压缩 + 按需分页读取，否则早晚要还性能债。
...
- MySQL 把 Collation（比较规则）和 Character Set（存储规则）强行解耦，但又允许它们在会话、数据库、表、字段、视图甚至连接（Connection）级别有不同的默认值。
    - MySQL: 新建个视图，它可能偷偷用了你客户端连接的 utf8mb4_0900_ai_ci ，而表上是 utf8_general_ci 或 utf8mb4_unicode_ci。两个一碰就报 Illegal mix of collations。这种报错在 Pg 里几乎见不到。
...
> 注意 utf8mb4_general_ci 的情况下 重读字符 ü 和 普通字母 u 会被MySQL视为等价字符，但是在Java应用中认为是两个字符因为字节不一样
